...
const Wallpaper = ({ api_req }) => {
    ...
    const { data: wallpaper = [] } = useQuery({
        ...
        cacheTime: 1000 * 60 * 15, // ✅ stored in memory
    })
                ...
                {wallpaper.map((val, index) => (
                    <SwiperSlide key={index}>
                        <div
                            ...
                            <h1 className='heading text-4xl md:text-[6vw] font-semibold w-[90%] md:w-[65%] leading-[100%] z-10'>{val.original_title || val.name}</h1>
                            <div className='z-10 text-xs md:text-[1.2vw] w-[85%] md:w-[45%]'>{val.overview.slice(0, 130)}
                                <span className='text-blue-300'>
                                    <Link>...more</Link>
                                </span>
                            </div>
                            <Link className='w-fit text-xl md:text-2xl h-fit px-4 md:px-7 md:py-4 py-3 bg-[#1A98FF] z-10 flex justify-center items-center rounded-md font-semibold transition-all duration-300 hover:shadow-[0_0_18px_4px_rgba(26,152,255,0.8)]'>Watch Now</Link>
                        </div>
                    </SwiperSlide>
                ))}
            </Swiper>
