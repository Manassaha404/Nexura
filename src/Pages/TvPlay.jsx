...
const TvPlay = () => {
    ...
    const getVideo = async () => {
        try {
            ...
            setvideo(data.results.filter((val, index) => (
                val.type === "Trailer"
            )
            ))
        } catch (e) {
    ...
    return data != null & seasonData != null ? (
        <div className='w-full h-fit pb-30 md:pb-0 bg-[#00050D]'>
            <div
                ...
                <h1 className='heading text-4xl md:text-[6vw] font-semibold w-[90%] md:w-[65%] leading-[100%] z-10'>{data.original_title || data.name}</h1>
                <div className='z-10 text- md:text-[1.2vw] w-[85%] md:w-[65%] font-semibold'>{isMobile ? data.tagline : seasonData.overview}
                </div>
                <div className='flex gap-2 md:gap-4 items-center mt-1 text-zinc-300 md:text-2xl  z-10'>
                    <p>{seasonData.air_date?.slice(0, 4)}</p>
                    {Math.floor(seasonData.episodes[0].runtime / 60) !== 0 && `${Math.floor(seasonData.episodes[0].runtime / 60)}h`} {seasonData.episodes[0].runtime % 60}min
                    <p className='text-yellow-300  font-semibold'>IMDB: {data.vote_average.toFixed(1)}</p>
                    <p className=''>{seasonData.episodes.length} episodes</p>
                </div>
                {!isMobile && video.length > 0 && <div className='trailer-box absolute right-10 w-[23vw] h-[13.5vw] overflow-hidden rounded-md shadow-[50px_50px_50px_#00050Dd0] bg-amber-500'>
                    <div className='size-[100%] absolute inset-0  z-10 bg-transparent   '></div>
                    <iframe frameBorder="0" className='trailer size-[100%] object-fill scale-153' src={`https://www.youtube.com/embed/${video[0].key}?autoplay=1&mute=1&controls=0&showinfo=0&rel=0&loop=1&playlist=${video[0].key}`} allowFullscreen></iframe>
                </div>}
                {data.genres ? <div className='flex gap-2 md:gap-4 items-center mt-1 text-zinc-300 md:text-2xl  z-10'>
                    {data.genres.map((val, index) => <p className='text-white font-semibold underline underline-offset-4'>{val.name}</p>)}
                </div> : ""}
                <select
                    value={seasons}
            ...
            <h1 className="text-white text-3xl md:text-5xl font-semibold slider-title md:px-27 px-5 md:mb-10 mb-6">Episodes</h1>
            {seasonData.episodes.map((val, index) => (
                <div onClick={() => {
                    ...
                    <div className="md:w-[60%] w-full  md:p-5 p-2 text-xl md:text-4xl font-semibold text-white flex flex-col justify-center md:gap-3">
                        <p>S{seasons} E{index + 1}</p>
                        {isMobile && <p className="text-xl text-zinc-300">{val.name}</p>}
                        {!isMobile && <>
                            <div className="flex gap-3 text-3xl">
                                <p>{formatDate(val.air_date)}</p>
                                <p>{Math.floor(val.runtime / 60) !== 0 && `${Math.floor(val.runtime / 60)}h`} {val.runtime % 60}min</p>
                            </div>
            ...
            <button
                ...
                className="back-btn text-2xl w-fit h-fit opacity-0 bg-amber-300 rounded-md  md:text-6xl  text-white absolute top-10 left-10 z-[1110000]"
            >
                <TiArrowBack  />
            </button>
            <HorizontalSlide title={"People Also Watch"} api_req={`tv/${id}/recommendations`} id={0} type={"tv"} />
            <HorizontalSlide title={"Similer"} api_req={`tv/${id}/similar`} id={1} type={"tv"} />
            {data.genres && data.genres.map((val, index) => (
                <HorizontalSlide
                    key={index}
                    title={`${val.name}`}
